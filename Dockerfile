...
RUN echo '#!/bin/bash' > start.sh && \
    ...
    echo 'exec daml start --sandbox-option --config=canton-runtime.conf --json-api-port=${PORT} --json-api-option --address=0.0.0.0 --start-navigator=no --sandbox-port=6865' >> start.sh && \
    chmod +x start.sh
